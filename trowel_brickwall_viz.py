...
OUTPUT_FILE_PATH = PROJECT_ROOT
INPUT_JSON_PATH = os.path.join(PROJECT_ROOT, "data", "brick_wall_side_surface_labels", "frame_0000.json")
...
def get_brickwall_side_surf_polygon_coord(root_path, json_file_path, output_path):
    ...
    try:
        ...
        for shape in data['shapes']:
            ...
            output_path = os.path.join(output_path, f"{label}.npy")
            
            # Save the NumPy array to its own file
            np.save(output_path, vertices_array)
            
            print(f" - Saved vertices for '{label}' to '{os.path.basename(output_path)}'")
...
if __name__ == "__main__":
    ...
    get_brickwall_side_surf_polygon_coord(PROJECT_ROOT, INPUT_JSON_PATH, PROJECT_ROOT)
    ...
    brickwall_side_surface_vertices = np.load("brick_wall_side_surface.npy", allow_pickle=True)
    brickwall_side_surface_plane = []
    brickwall_side_surface_mask = np.zeros((height, width), dtype=np.int8)
    cv2.fillPoly(brickwall_side_surface_mask, [brickwall_side_surface_vertices],1)
    rows_brickwall, cols_brickwall = np.where(brickwall_side_surface_mask == 1)
    brickwall_side_surface_2d = np.vstack((cols_brickwall, rows_brickwall)).T
    brickwall_side_surface_3d = unproject_points(brickwall_side_surface_2d, all_depth_maps[0], INTRINSICS)
    ...
    plotter.add_points(brickwall_side_surface_3d, style='points', color='#D95319', 
